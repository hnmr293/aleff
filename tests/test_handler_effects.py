...
When a handler function performs an effect, it should be dispatched to an
enclosing (outer) handler on the stack, enabling handler-to-handler
...
class TestSyncHandlerPerformsEffect:
    def test_handler_performs_different_outer_effect(self):
        ...
        def _handle_b(k: Resume[str, str]):
            val = effect_a()  # perform outer effect from handler
            return k(val)
    ...
    def test_handler_performs_same_effect_caught_by_outer(self):
        ...
        def _handle_a_inner(k: Resume[str, str]):
            val = effect_a()  # should go to outer, not self
            return k(val)
    ...
    def test_effect_before_resume(self):
        """Handler fn performs outer effect before calling k()."""
    ...
    def test_effect_after_resume(self):
        """Handler fn performs outer effect after k() returns.

        handler_b resumes caller with 0, caller returns 0.
        Then handler_b performs effect_a, outer handler resumes with 10.
        handler_b returns 0 + 10 = 10.
        """
        effect_a: Effect[[], int] = effect("a")
        effect_b: Effect[[], int] = effect("b")
        log: list[str] = []
    ...
    def test_handler_performs_multiple_outer_effects(self):
        """Handler fn performs multiple outer effects sequentially."""
        effect_a: Effect[[], int] = effect("a")
        effect_b: Effect[[], int] = effect("b")
    ...
    def test_nested_three_levels(self):
        """Three levels of handlers; middle handler fn performs outermost effect."""
        effect_a: Effect[[], str] = effect("a")
        effect_b: Effect[[], str] = effect("b")
        ...
        @h_b.on(effect_b)
        def _handle_b(k: Resume[str, str]):
            val = effect_a()  # perform outermost effect
            return k(val + "_B")
    ...
    def test_outer_handler_aborts(self):
        """Outer handler doesn't call k() -> inner handler fn and caller aborted."""
        effect_a: Effect[[], str] = effect("a")
        effect_b: Effect[[], str] = effect("b")
        ...
        def _handle_b(k: Resume[str, str]):
            ...
            effect_a()  # outer handler will abort
            inner_k_called = True  # should not reach here
    ...
    def test_shallow_handler_fn_performs_outer_effect(self):
        """Shallow handler fn can perform effects handled by outer handler."""
        effect_a: Effect[[], str] = effect("a")
        effect_b: Effect[[], str] = effect("b")
    ...
    def test_handler_effect_not_handled_raises(self):
        """Handler fn performs an effect with no outer handler -> EffectNotHandledError."""
        effect_a: Effect[[], str] = effect("a")
        effect_b: Effect[[], str] = effect("b")
        ...
        @h.on(effect_b)
        def _handle_b(k: Resume[str, str]):
            effect_a()  # no handler for effect_a
            return k("unreachable")
    ...
    def test_handler_effect_with_args(self):
        """Handler fn performs an outer effect with arguments."""
    ...
    def test_caller_effects_still_work_after_handler_effect(self):
        """After handler fn performs outer effect and resumes, caller's subsequent
        effects are still caught by inner handler."""
        effect_a: Effect[[], int] = effect("a")
        effect_b: Effect[[], int] = effect("b")
        call_count = 0
        ...
        def caller() -> int:
            first = effect_b()
            second = effect_b()  # should still be caught by inner handler
            return first + second

        result = h_outer(lambda: h_inner(caller))
        assert result == 102  # 100 + 2
...
class TestAsyncHandlerPerformsEffect:
    ...
    async def test_async_handler_performs_outer_effect(self):
        """Async inner handler fn performs effect caught by outer async handler."""
        effect_a: Effect[[], str] = effect("a")
        effect_b: Effect[[], str] = effect("b")
    ...
    async def test_async_handler_performs_same_effect(self):
        """Async handler for effect_a performs effect_a -> caught by outer."""
        effect_a: Effect[[], str] = effect("a")
    ...
    async def test_async_effect_before_resume(self):
        """Async handler fn performs outer effect before await k()."""
        effect_a: Effect[[], int] = effect("a")
        effect_b: Effect[[], int] = effect("b")
    ...
    async def test_async_outer_handler_aborts(self):
        """Async outer handler doesn't await k() -> inner handler fn aborted."""
        effect_a: Effect[[], str] = effect("a")
        effect_b: Effect[[], str] = effect("b")
        inner_k_called = False
...
class TestMixedHandlerEffects:
    ...
    async def test_sync_handler_fn_performs_effect_caught_by_async_outer(self):
        """Sync inner handler fn performs effect caught by outer async handler."""
        effect_a: Effect[[], str] = effect("a")
        effect_b: Effect[[], str] = effect("b")
