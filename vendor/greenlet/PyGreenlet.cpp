...
green_switch(PyGreenlet* self, PyObject* args, PyObject* kwargs)
{
    ...
    // The wrapper frame belongs to the greenlet that is switching out, not
    // necessarily `self` (which is the switch target).
    GET_THREAD_STATE().state().borrow_current()->preserve_switch_call_refs(
        args,
        kwargs
    );
    ...
    try {
        OwnedObject switched(self->pimpl->g_switch());
        GET_THREAD_STATE().state().borrow_current()->consume_cloned_switch_call_refs();
        OwnedObject result(single_result(switched));
#ifndef NDEBUG
...
PyDoc_STRVAR(
    ...
    "clone() -> greenlet\n"
    "\n"
    "Clone a suspended greenlet without invoking its run callable.\n"
    "The clone is valid only in the same thread and interpreter as the\n"
    "source greenlet. The source remains suspended and may be cloned\n"
    "again for another sequential resume.\n");

static PyObject*
green_clone(PyGreenlet* self, PyObject* args)
{
    if (!PyArg_ParseTuple(args, ":clone")) {
        return nullptr;
    }

#if !GREENLET_PY312 || GREENLET_PY315 || defined(Py_GIL_DISABLED)
    PyErr_SetString(
        mod_globs->PyExc_GreenletError,
        "clone() requires GIL-enabled CPython 3.12 through 3.14"
    );
    return nullptr;
#else

    PyCriticalObjectSection cs(self);
    Greenlet* source = self->pimpl;
    ThreadState& state = GET_THREAD_STATE().state();
    if (!source->started() || !source->active() || state.is_current(source->self())
        || source->is_currently_running_in_some_thread()) {
        PyErr_SetString(mod_globs->PyExc_GreenletError,
                        "clone() requires a suspended greenlet");
        return nullptr;
    }
    if (!source->belongs_to_thread(&state)) {
        PyErr_SetString(mod_globs->PyExc_GreenletError,
                        "cannot clone a greenlet from another thread");
        return nullptr;
    }
    if (!source->belongs_to_interpreter(PyInterpreterState_Get())) {
        PyErr_SetString(mod_globs->PyExc_GreenletError,
                        "cannot clone a greenlet from another interpreter");
        return nullptr;
    }

    PyGreenlet* result = green_new(&PyGreenlet_Type, nullptr, nullptr);
    if (!result) {
        return nullptr;
    }
    try {
        if (source->clone_state_to(*result->pimpl) < 0) {
            Py_DECREF(result);
            if (!PyErr_Occurred()) {
                PyErr_NoMemory();
            }
        ...
        // Context objects are mutable. A clone gets the same context state at
        // the suspension point, but must not let one shot mutate another.
        OwnedObject source_context = source->context();
        if (source_context.borrow() == Py_None) {
            result->pimpl->context(BorrowedObject(Py_None));
        }
        else {
            OwnedObject clone_context = OwnedObject::consuming(
                PyObject_CallMethod(source_context.borrow(), "copy", nullptr));
            if (!clone_context) {
                Py_DECREF(result);
                return nullptr;
            }
            result->pimpl->context(BorrowedObject(clone_context.borrow()));
        }
...
static PyMethodDef green_methods[] = {
    ...
    {.ml_name="clone", .ml_meth=(PyCFunction)green_clone, .ml_flags=METH_VARARGS, .ml_doc=green_clone_doc},
