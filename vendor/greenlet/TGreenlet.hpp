...
namespace greenlet
{
    ...
    public:
        ExceptionState();
        void clone_from(const ExceptionState& other) noexcept;
    ...
    private:
        ...
        int cloned_current_frame_stacktop;
    ...
    public:
        ...
        void clear_cloned_state(const StackState& stack) noexcept;
    ...
    public:
        ...
        int clone_from(const StackState& other) noexcept;
        void relocate_pointer(const void* source, void* target) noexcept;
        void relocate_stack_range(
            const void* source,
            size_t size,
            void* target
        ) noexcept;
        ...
        const void* find_pointer_in_range(
            const void* begin,
            const void* end
        ) const noexcept;
    };
    ...
    public:
        ...
        void clone_from(const SwitchingArgs& other)
        {
    ...
    private:
        ...
        PyInterpreterState* const _interpreter;
    private:
    ...
    protected:
        ...
        // PyCFunction's vectorcall wrapper owns the args/kwargs objects while
        // green_switch is suspended. A raw native-stack clone duplicates the
        // eventual DECREF without duplicating that ownership, so cloned
        // greenlets keep one transferable reference for each wrapper object.
        PyObject* suspended_switch_args = nullptr;
        PyObject* suspended_switch_kwargs = nullptr;
        OwnedObject cloned_switch_args_ref;
        OwnedObject cloned_switch_kwargs_ref;
    ...
    public:
        ...
        virtual int clone_state_to(Greenlet& target) const;

        inline bool belongs_to_interpreter(PyInterpreterState* interpreter) const noexcept
        {
            return this->_interpreter == interpreter;
        }

        virtual void did_resume_cloned_state() noexcept
        {}
        ...
        inline void preserve_switch_call_refs(PyObject* args, PyObject* kwargs)
        {
            this->suspended_switch_args = args;
            this->suspended_switch_kwargs = kwargs;
        }

        inline void clone_switch_call_refs_from(const Greenlet& source)
        {
            this->suspended_switch_args = source.suspended_switch_args;
            this->suspended_switch_kwargs = source.suspended_switch_kwargs;
            this->cloned_switch_args_ref = source.suspended_switch_args;
            this->cloned_switch_kwargs_ref = source.suspended_switch_kwargs;
        }

        inline void consume_cloned_switch_call_refs()
        {
            // Transfer the references to the copied vectorcall frame. Its
            // normal return path will DECREF them exactly once.
            this->cloned_switch_args_ref.relinquish_ownership();
            this->cloned_switch_kwargs_ref.relinquish_ownership();
            this->suspended_switch_args = nullptr;
            this->suspended_switch_kwargs = nullptr;
        }
...
public:
    ...
    public:
        ...
        virtual void did_resume_cloned_state() noexcept
        {
            // Once installed in PyThreadState, CPython owns and clears the
            // cloned interpreter-frame references through its normal paths.
            this->_cloned_state = false;
        }
