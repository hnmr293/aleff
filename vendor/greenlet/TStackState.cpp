...
int
StackState::clone_from(const StackState& other) noexcept
{
    assert(!this->started());
    this->_stack_start = other._stack_start;
    this->stack_stop = other.stack_stop;
    ...
    if (other._stack_saved == 0) {
        return 0;
    }

    this->stack_copy = static_cast<char*>(PyMem_Malloc(other._stack_saved));
    if (!this->stack_copy) {
        this->_stack_saved = 0;
        return -1;
    }
    memcpy(this->stack_copy, other.stack_copy, other._stack_saved);
    this->relocate_stack_range(
        other.stack_copy,
        static_cast<size_t>(other._stack_saved),
        this->stack_copy
    );
    return 0;
}
...
void
StackState::relocate_pointer(const void* source, void* target) noexcept
{
    if (!source || source == target || this->_stack_saved == 0) {
        return;
    }
    uintptr_t old_value = reinterpret_cast<uintptr_t>(source);
    uintptr_t new_value = reinterpret_cast<uintptr_t>(target);
    for (size_t offset = 0;
         offset + sizeof(uintptr_t) <= static_cast<size_t>(this->_stack_saved);
         offset += sizeof(uintptr_t)) {
        uintptr_t value;
        memcpy(&value, this->stack_copy + offset, sizeof(value));
        if (value == old_value) {
            memcpy(this->stack_copy + offset, &new_value, sizeof(new_value));
        }
    }
}

void
StackState::relocate_stack_range(
    const void* source,
    size_t size,
    void* target
) noexcept
{
    if (!source || !target || size == 0 || this->_stack_saved == 0) {
        return;
    }
    uintptr_t old_begin = reinterpret_cast<uintptr_t>(source);
    uintptr_t old_end = old_begin + size;
    uintptr_t new_begin = reinterpret_cast<uintptr_t>(target);
    for (size_t offset = 0;
         offset + sizeof(uintptr_t) <= static_cast<size_t>(this->_stack_saved);
         offset += sizeof(uintptr_t)) {
        uintptr_t value;
        ...
        if (value >= old_begin && value < old_end) {
            uintptr_t relocated = new_begin + (value - old_begin);
            memcpy(this->stack_copy + offset, &relocated, sizeof(relocated));
        }
    }
}
...
const void*
StackState::find_pointer_in_range(const void* vbegin, const void* vend) const noexcept
{
    if (!this->stack_copy || this->_stack_saved < (intptr_t)sizeof(uintptr_t)) {
        return nullptr;
    }
    const uintptr_t begin = reinterpret_cast<uintptr_t>(vbegin);
    const uintptr_t end = reinterpret_cast<uintptr_t>(vend);
    uintptr_t best = 0;
    uintptr_t address = reinterpret_cast<uintptr_t>(this->_stack_start);
    uintptr_t aligned = (address + sizeof(uintptr_t) - 1)
        & ~(static_cast<uintptr_t>(sizeof(uintptr_t) - 1));
    for (size_t offset = aligned - address;
         offset + sizeof(uintptr_t) <= static_cast<size_t>(this->_stack_saved);
         offset += sizeof(uintptr_t)) {
        uintptr_t value = 0;
        memcpy(&value, this->stack_copy + offset, sizeof(value));
        if (value >= begin && value <= end
            && (value - begin) % sizeof(PyObject*) == 0
            && value > best) {
            best = value;
        }
    }
    return reinterpret_cast<const void*>(best);
}
