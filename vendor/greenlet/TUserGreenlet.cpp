...
UserGreenlet::~UserGreenlet()
{
    if (this->_cloned_state) {
        this->python_state.clear_cloned_state(this->stack_state);
    }
...
int
UserGreenlet::clone_state_to(Greenlet& raw_target) const
{
    UserGreenlet& target = static_cast<UserGreenlet&>(raw_target);
    if (target.stack_state.clone_from(this->stack_state) < 0) {
        return -1;
    }
    // From this point on, target owns partially cloned state and its
    // destructor must roll it back if any later step fails.
    target._cloned_state = true;
    target.exception_state.clone_from(this->exception_state);
    if (target.python_state.clone_from(
            this->python_state,
            this->stack_state,
            target.stack_state
        ) < 0) {
    ...
    target.stack_state.relocate_stack_range(this, sizeof(*this), &target);
    target.stack_state.relocate_pointer(
        this->self().borrow(),
        target.self().borrow()
    );
    target.switch_args.clone_from(this->switch_args);
    target.clone_switch_call_refs_from(*this);
    target._main_greenlet = this->_main_greenlet;
    target._run_callable = this->_run_callable;
